    ...
    <div class="mx-2">
      ...
      <form action="{{ route('testimonials.update', $testimonials->id) }}" method="post" class="px-md-5"enctype="multipart/form-data">
        ...
        <div class="form-group mb-3 row">
          ...
          <div class="col-md-10">
            <input type="file" class="form-control" style="padding: 0.7rem; margin-bottom: 10px;" name="image" value="{{old('image', $testimonials['image'])}}"/>
            <img src="{{asset("adminassets/images/". $testimonials['image']) }}" alt="" style="width: 10rem;">
           @error('image')
          <div class="alert alert-warning">{{$message}}</div>
          @enderror
          </div>
